...
function HandParticles({ activeFingers }: { activeFingers: number }) {
  ...
  useFrame((state) => {
    ...
    const ringArray = ringRef.current.geometry.attributes.position
      .array as Float32Array;
  ...
  return (
    <>
      <points ref={pointsRef}>
        <bufferGeometry>
          {/* FIX: Use 'args' prop for BufferAttribute constructor */}
...
export default function SurprisePage() {
  ...
  // UBAH TANGGAL JADIAN KE TANGGAL LAHIR
  const TANGGAL_JADIAN = "2002-03-31"; // <-- SUDAH DIUBAH KE 31 Maret 2002
  // UBAH PASSWORD JADIAN DARI DDMMYY TANGGAL LAHIR
  const PASSWORD_JADIAN = "310302"; // <-- SUDAH DIUBAH KE 310302
  ...
  const milestones = [
    {
      date: "31 Maret 2002", // Tanggal lahir
      event: "Hari lahir Duyulll! ❤️", // Peristiwa spesial
      icon: <Gift className="w-4 h-4 text-pink-500" />, // Ikon kado
    },
    {
      date: "Aku lupa hihi",
      event: "First Date: nonton bioskop! 🍿",
      icon: <MapPin className="w-4 h-4" />,
    },
    {
      date: "17 April 2025",
      event: "Hari kita pertama pacaran ❤️",
      icon: <MessageCircleHeart className="w-4 h-4" />,
    },
    {
      date: "31 Maret 2026",
      event: "Selamat bertambah TUA (24 TAHUN)! 🎉",
      icon: <StarsIcon className="w-4 h-4 text-yellow-500" />,
    },
    {
      date: "Sekarang dan Selamanya",
      event: "Menua Bersama ✨",
      ...
      const loadMediaPipe = async () => {
        ...
        if (camVideoRef.current) {
          cameraInstance = new camUtils.Camera(camVideoRef.current, {
            onFrame: async () => {
              if (camVideoRef.current)
                await hands.send({ image: camVideoRef.current });
            },
            width: 640,
            height: 480,
          });
        ...
        {show3DSpace && (
          ...
          >
            ...
            <div className="absolute top-10 left-6 z-[110] space-y-3 bg-black/40 p-5 rounded-3xl border border-white/10 backdrop-blur-md text-white">
              <p
                className={`flex items-center gap-3 text-[10px] transition-all ${
                  activeFingers === 1
                    ? "text-pink-400 font-bold scale-110"
                    : "opacity-30"
                }`}
              >
                <Heart size={12} /> 1: Cinta
              </p>
              <p
                className={`flex items-center gap-3 text-[10px] transition-all ${
                  activeFingers === 2
              ...
              >
                <Sparkles size={12} /> 4: Reflection of Love
              </p>
            </div>
            ...
            <div className="absolute bottom-10 left-1/2 -translate-x-1/2 text-center">
              <button
                onClick={() => setShow3DSpace(false)}
                className="bg-white/10 backdrop-blur-md text-white px-8 py-3 rounded-full border border-white/20 flex items-center gap-2 hover:bg-white/20 transition-all"
              >
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
              <motion.div
                ...
                className="text-center bg-black/20 backdrop-blur-3xl p-6 md:p-8 rounded-[3rem] border border-white/20 mx-4"
              >
                ...
                <div className="flex gap-3 md:gap-5 text-white mb-6 justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-[7px] md:text-[8px] tracking-widest uppercase">
                      Hari
                    </p>
                  </div>
                  <div className="text-center">
                  ...
                  <div className="text-center">
                    <p className="text-3xl md:text-4xl font-black">
                      {timeTogether.mins.toString().padStart(2, "0")}
                    </p>
                    <p className="text-[7px] md:text-[8px] tracking-widest uppercase">
                      Menit
                    </p>
                  ...
                  <div className="text-center text-pink-400">
                    <p className="text-3xl md:text-4xl font-black">
                      {timeTogether.secs.toString().padStart(2, "0")}
                    </p>
                    <p className="text-[7px] md:text-[8px] tracking-widest uppercase font-bold">
                      Detik
                    </p>
                  </div>
                </div>
                {/* ------------------------------------------- */}
                <h2 className="text-4xl font-black text-white italic uppercase mb-2">
                  I Love You!
                </h2>
                ...
                <button
                  ...
                  className="bg-gradient-to-r from-pink-500 to-purple-600 px-8 py-4 rounded-2xl text-white font-bold flex items-center gap-3 mx-auto shadow-lg animate-bounce"
                >
                  <Camera size={20} /> MAGIC 3D SPACE 🪄
                </button>
              </motion.div>
              <button
                onClick={() => setIsLovePage(false)}
                className="mt-10 text-white/50 text-xs tracking-[0.3em] uppercase hover:text-white transition-colors"
              >
                Tutup Video
              </button>
            </div>
        ...
        <div className="z-20 w-full max-w-lg mb-24">
          {!showMessage ? (
            <motion.div
              initial={{ scale: 0.9 }}
              animate={{ scale: 1 }}
              className="bg-white/5 backdrop-blur-xl p-10 rounded-[3rem] text-center border border-white/10 max-w-md mx-auto text-white"
            >
              <Gift className="w-20 h-20 text-pink-400 mx-auto mb-6 animate-pulse" />
              {/* UBAH UCAPAN SELAMAT */}
              <h1 className="text-2xl font-black mb-8 italic tracking-tight">
                Happy Birthday Duyulll! ✨ {/* <-- SUDAH DIUBAH */}
              </h1>
              <button
                onClick={handleStart}
                className="bg-pink-500 hover:bg-pink-600 transition-colors text-white w-full py-4 rounded-2xl font-extrabold uppercase shadow-xl shadow-pink-500/20"
              >
                BUKA HADIAHMU ❤️
              </button>
            </motion.div>
              ...
              <AnimatePresence mode="wait">
                {activeTab === "home" && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -20 }}
                    className="bg-white/95 p-6 rounded-[2.5rem] shadow-2xl"
                  >
                    {isLetterLocked ? (
                      <div className="text-center py-6">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.value === PASSWORD_JADIAN) {
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-gray-600 italic text-sm leading-relaxed mb-8 px-4">
                          Selamat ulang tahun yang ke-24 ya {NAMA_DIA} 🎉
                          Makasih banget karena sudah hadir dan jadi bagian
                          penting di hidup aku. Semoga di umur yang sekarang
                          kamu selalu diberi kesehatan, hati yang tenang, dan
                          rezeki yang terus mengalir tanpa putus. Semoga semua
                          hal baik pelan-pelan datang ke kamu, apa yang lagi
                          diperjuangkan bisa tercapai satu per satu, dan kamu
                          selalu dikelilingi orang-orang yang tulus sayang sama
                          kamu. Jangan lupa tetap jadi diri kamu yang hangat,
                          kuat, dan selalu berusaha jadi lebih baik setiap
                          harinya. Aku cuma mau kamu tahu, kehadiran kamu itu
                          berarti banget buat aku. Semoga tahun ini membawa
                          lebih banyak bahagia, tawa, dan cerita indah yang bisa
                          kita lewati bareng. 🤍
                        </p>
                ...
                {activeTab === "milestone" && (
                  <motion.div
                    ...
                    className="bg-white/95 p-6 rounded-[2.5rem] shadow-2xl"
                  >
                    <h2 className="text-center font-black text-pink-500 mb-6 uppercase text-xs tracking-[0.2em]">
                      Our Journey
                    </h2>
                    <div className="space-y-4 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                      {milestones.map((m, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-4 bg-pink-50/50 p-4 rounded-3xl border border-pink-100/50"
                        >
                          <div className="bg-white p-3 rounded-2xl text-pink-500 shadow-sm">
                            {m.icon}
                          </div>
                          <div>
                            <p className="text-[10px] font-bold text-pink-400 uppercase tracking-tighter">
                              {m.date}
                            </p>
                ...
                {activeTab === "wishlist" && (
                  <motion.div
                    ...
                    initial={{ y: 20, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    className="bg-white/95 p-6 rounded-[2.5rem] shadow-2xl min-h-[420px] flex flex-col"
                  >
                    <h2 className="text-center font-black text-pink-500 mb-4 uppercase text-xs tracking-[0.2em]">
                      Wishlist Kita ✨
                    </h2>
                    <div className="flex gap-2 mb-6">
                      ...
                      <button
                        ...
                        className="bg-pink-500 text-white p-4 rounded-2xl shadow-lg shadow-pink-500/20 hover:scale-105 transition-transform"
                      >
                        <Plus size={20} />
                      </button>
                    </div>
                    <div className="flex-1 overflow-y-auto space-y-3 max-h-[260px] pr-2 custom-scrollbar">
                      {wishes.length === 0 && (
                        <p className="text-center text-gray-300 text-xs italic mt-10">
                          Belum ada harapan...
                        </p>
                      )}
                      {wishes.map((w, i) => (
                        <motion.div
                          ...
                          key={i}
                          className="flex justify-between items-center bg-white border border-pink-50 p-4 rounded-2xl shadow-sm"
                        >
                ...
                {activeTab === "gallery" && (
                  <motion.div
                    ...
                    initial={{ y: 20, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    className="bg-white/95 p-4 rounded-[2.5rem] shadow-2xl"
                  >
                    ...
                    <div className="flex justify-between mt-6 px-4 items-center">
                      <button
                        onClick={() =>
                          setCurrentFoto(
                            (p) => (p - 1 + albumFoto.length) % albumFoto.length
                          )
                        }
                        className="text-pink-500 p-2 bg-pink-50 rounded-full hover:bg-pink-100 transition-colors"
                      >
                        <ChevronLeft size={24} />
                      </button>
                      <div className="text-center">
                        <span className="font-black text-pink-500 text-sm tracking-widest">
                          {currentFoto + 1} / {albumFoto.length}
                        </span>
                      ...
                      <button
                        onClick={() =>
                          setCurrentFoto((p) => (p + 1) % albumFoto.length)
                        }
                        className="text-pink-500 p-2 bg-pink-50 rounded-full hover:bg-pink-100 transition-colors"
                      >
